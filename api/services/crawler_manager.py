...
def _path_helpers():
    """延迟导入 desktop.paths（打包/开发态均可）。"""
    try:
        from desktop import paths as _p  # type: ignore
        return _p
    except Exception:
        return None
...
class CrawlerManager:
    ...
    def __init__(self):
        ...
        # 打包态下：worker 由同一个 exe 以 --run-crawler 参数拉起
    ...
    def _build_command(self, config: CrawlerStartRequest) -> list:
        ...
        打包态： [MediaCrawler.exe, --run-crawler, ...]
        开发态： [python, main.py, ...] 或 uv run
        """
        if self._frozen:
            cmd = [sys.executable, "--run-crawler"]
        else:
            # 开发态：优先直接使用当前解释器，避免依赖 uv
        ...
        # 统一数据落地目录（打包态 -> %APPDATA%\MediaCrawler）
        paths = _path_helpers()
        if paths is not None:
            try:
                paths.ensure_runtime_dirs()
                cmd.extend(["--save_data_path", str(paths.user_data_root())])
            except Exception:
                pass
