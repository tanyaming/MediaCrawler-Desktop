...
async def start_crawler(request: CrawlerStartRequest):
    """Start crawler task"""
    # search 模式校验：关键词必填，避免静默回退到默认关键词
    if request.crawler_type.value == "search" and not (request.keywords or "").strip():
        raise HTTPException(status_code=400, detail="搜索模式需要至少一个关键词")
