...
class WeiboCrawler(AbstractCrawler):
    ...
    async def start(self):
        ...
        async with async_playwright() as playwright:
            ...
            await self._goto_page(self.index_url)
            ...
            if not await self.wb_client.pong():
                ...
                await self._goto_page(self.mobile_index_url)
    ...
    async def _goto_page(self, url: str):
        """打开页面，使用 domcontentloaded 等待并自动重试，避免网络慢时超时崩溃。"""
        ...
        for attempt in range(1, 4):
            try:
                ...
                utils.logger.info(f"[WeiboCrawler] 页面加载成功 (attempt {attempt}): {url}")
                return
            except Exception as e:
                utils.logger.warning(f"[WeiboCrawler] 页面加载失败 (attempt {attempt}): {e}")
                if attempt < 3:
