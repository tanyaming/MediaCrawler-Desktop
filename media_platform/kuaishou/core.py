...
class KuaishouCrawler(AbstractCrawler):
    ...
    async def _goto_index_page(self):
        """打开快手首页，使用 domcontentloaded 等待并自动重试，避免网络慢时超时崩溃。"""
        ...
        for attempt in range(1, 4):
            try:
                ...
                utils.logger.info(f"[KuaishouCrawler] 首页加载成功 (attempt {attempt})")
                return
            except Exception as e:
                utils.logger.warning(f"[KuaishouCrawler] 首页加载失败 (attempt {attempt}): {e}")
                if attempt < 3:
