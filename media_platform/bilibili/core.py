...
from tools.browser_launcher import detect_system_browser
...
class BilibiliCrawler(AbstractCrawler):
    ...
    async def _goto_index_page(self):
        """打开 B 站首页，使用 domcontentloaded 等待并自动重试，避免网络慢时超时崩溃。"""
        timeout = getattr(config, "PAGE_LOAD_TIMEOUT_MS", 60000)
        for attempt in range(1, 4):
            try:
                await self.context_page.goto(self.index_url, wait_until="domcontentloaded", timeout=timeout)
                utils.logger.info(f"[BilibiliCrawler] 首页加载成功 (attempt {attempt})")
                return
            except Exception as e:
                utils.logger.warning(f"[BilibiliCrawler] 首页加载失败 (attempt {attempt}): {e}")
                if attempt < 3:
    ...
    async def launch_browser(
        ...
        # 打包后 exe 不带 Playwright 浏览器内核，显式指定本机 Chrome/Edge
        executable_path = detect_system_browser()
        if config.SAVE_LOGIN_STATE:
            ...
            # type: ignore
            browser = await chromium.launch(headless=headless, proxy=playwright_proxy, executable_path=executable_path)
