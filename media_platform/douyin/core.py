...
class DouYinCrawler(AbstractCrawler):
    ...
    async def _goto_index_page(self) -> None:
        """
        打开抖音首页。使用 domcontentloaded 而非 load 作为等待条件，
        避免因页面资源过多/网络较慢导致默认 30 秒 load 超时崩溃。
        失败时自动重试并延长超时时间。
        """
        ...
        for attempt in range(1, 4):
            try:
                ...
                utils.logger.info(f"[DouYinCrawler] 首页加载成功 (attempt {attempt})")
                return
            except Exception as e:
                utils.logger.warning(
                    f"[DouYinCrawler] 首页加载失败 (attempt {attempt}): {e}"
                )
                if attempt < 3:
