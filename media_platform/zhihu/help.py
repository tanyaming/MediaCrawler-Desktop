...
def _resolve_libs(name: str) -> str:
    """解析 libs/ 下 JS 文件的绝对路径（兼容打包态与开发态）。"""
    import os

    try:
        from desktop.paths import resource_path

        return resource_path(os.path.join("libs", name))
    except Exception:
...
def sign(url: str, cookies: str) -> Dict:
    ...
    if not ZHIHU_SGIN_JS:
        with open(_resolve_libs("zhihu.js"), mode="r", encoding="utf-8-sig") as f:
