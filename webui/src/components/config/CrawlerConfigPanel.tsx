...
import { Database, Globe, KeyRound, MessageSquare, Play, Square } from 'lucide-react'
...
function Section({ title, description, icon: Icon, children, className = '' }: SectionProps) {
  return (
    <section className={`rounded-xl bg-cyber-bg-panel border border-cyber-border-subtle shadow-sm overflow-hidden ${className}`}>
      <header className="px-4 py-3 border-b border-cyber-border-subtle flex items-center gap-3 bg-gradient-to-r from-cyber-neon-cyan/8 to-transparent">
        <div className="h-9 w-9 rounded-lg bg-gradient-to-br from-cyber-neon-cyan to-cyber-neon-purple flex items-center justify-center flex-shrink-0 shadow-sm">
          <Icon className="h-4.5 w-4.5 text-white" />
        </div>
        <div className="min-w-0">
          <div className="text-sm font-semibold text-cyber-text-primary tracking-tight">
            {title}
          </div>
          <div className="text-[11px] text-cyber-text-muted leading-snug truncate">
            {description}
          </div>
        </div>
      </header>
  ...
  return (
    <div className="space-y-2">
      <div className="space-y-0.5">
        <Label className="text-xs font-medium text-cyber-text-secondary">
          {label}
        </Label>
        {hint ? (
...
function KeywordInput({ value, onChange, placeholder, disabled }: KeywordInputProps) {
  // 普通文本框：直接输入，多个关键词用逗号/空格分隔，实时保存，无需回车变标签
  return (
    <div className="space-y-2">
      <Input
        value={value}
...
export function CrawlerConfigPanel() {
  ...
  const handleStart = () => {
    // search 模式校验：关键词不能为空，避免静默回退到默认值
    if (config.crawler_type === 'search' && !config.keywords.trim()) {
      toast.error(t('field.keywordsRequired') || '请输入至少一个搜索关键词')
      return
    }
    startCrawler(config)
  }
