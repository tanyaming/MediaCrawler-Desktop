...
class CDPBrowserManager:
    ...
    async def _launch_browser(self, browser_path: str, headless: bool):
        ...
        if config.SAVE_LOGIN_STATE:
            # 优先使用重定向后的可写目录（desktop.paths），避免 frozen 下 cwd 不可写
            try:
                from desktop.paths import browser_data_root
                base = browser_data_root()
            except Exception:
                base = os.path.join(os.getcwd(), "browser_data")
    ...
    async def add_stealth_script(self, script_path: str = "libs/stealth.min.js"):
        ...
        if not os.path.isabs(script_path):
            try:
                ...
                resolved_path = resource_path(script_path)
            except Exception:
                resolved_path = script_path
        if self.browser_context and os.path.exists(resolved_path):
            try:
                await self.browser_context.add_init_script(path=resolved_path)
                utils.logger.info(
                    f"[CDPBrowserManager] Added anti-detection script: {resolved_path}"
                )
            except Exception as e:
                utils.logger.warning(f"[CDPBrowserManager] Failed to add anti-detection script: {e}")
        elif self.browser_context:
            utils.logger.warning(
                f"[CDPBrowserManager] Anti-detection script not found: {resolved_path}"
            )
