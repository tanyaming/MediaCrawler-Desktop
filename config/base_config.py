...
# 页面加载超时时间（毫秒），用于 Playwright page.goto。
# 网络较慢的机器上默认 30 秒容易超时，因此默认提高到 60 秒。
...
# 是否连接用户已打开的浏览器（用户需手动开远程调试端口 9222），而不是程序自动启动一个新浏览器实例。
# 桌面版场景：改为 False，让程序自动启动本机 Chrome/Edge 并自动开调试端口，客户无需任何手动操作。
# 如需复用你日常带着所有登录态的浏览器，再手动改为 True。
